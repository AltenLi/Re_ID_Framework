function [neighborIds,neighborScores,neighborDistances]  = gmmknnclassify_part_score(querymodels, gallerymodels, cur_part, SR_EMD_para, useFLIP)
% gmmknnclassify
%
...
if ~useFLIP
    ...
    for i=1:numQueryVectors
        for j=1:numGalleryVectors
            ...
            dist(i,j) = SR_EMD(querymodels{i}.features{k}, gallerymodels{j}.features{k},  SR_EMD_para);
        end
    end
    ...
    for i=1:numQueryVectors/2
        for j=1:numGalleryVectors/2
            k=cur_part;%calc cur_part distance
            tmpdist=ones(1,4);
            tmpdist(1)=SR_EMD(querymodels{i}.features{k},                                  gallerymodels{j}.features{k},  SR_EMD_para);
            tmpdist(2)=SR_EMD(querymodels{i+numQueryVectors/2}.features{k}, gallerymodels{j}.features{k},  SR_EMD_para);
            tmpdist(3)=SR_EMD(querymodels{i}.features{k},                                  gallerymodels{j+numGalleryVectors/2}.features{k},  SR_EMD_para);
            tmpdist(4)=SR_EMD(querymodels{i+numQueryVectors/2}.features{k}, gallerymodels{j+numGalleryVectors/2}.features{k},  SR_EMD_para);
